...
The wiki follows the **Obsidian LLM-wiki standard** (raw source: `docs/raw/llm-wiki-obsidian-standard.md`; hard rules: behavioral rule 18; essentials: `CLAUDE.md` → Frontmatter convention). Routine ticks (`[ ]` → `[~]` → `[x]`, checking off a todo, appending a log line) are documented in `tdd-loop`. This skill covers: **placement**, the **templates**, the **facet/ontology tables**, and **inline-vs-maintainer routing**.
