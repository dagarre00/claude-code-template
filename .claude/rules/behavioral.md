...
19. **Branch for code changes; living wiki commits directly on develop (or current branch).**
    ...
    - **Living documentation & operations (`docs/wiki/`, `docs/raw/`, `.claude/` config)** from maintenance commands (`/project:wiki-lint`, `/project:review`, `/project:wiki-ingest`, `/project:interview`, `/project:agent-scout`, `/project:handoff`, `/project:adversary`): when standing on `develop`, commit and push directly to `develop` (`git push -u origin develop`). If you are already on a `feat/*`/`fix/*`/`chore/*` branch whose work this belongs to, keep the doc edits on that branch. This prevents branch and PR proliferation for routine knowledge-base updates while preserving strict PR-gated integration for application code.
