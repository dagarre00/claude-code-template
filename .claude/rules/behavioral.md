...
18. **Obsidian LLM-wiki standard — hard rules.** These are not stylistic; violating them breaks rendering, the graph, or dedup. Inside `docs/wiki/`:
    ...
    - **Folders are a derived projection; frontmatter wins over path.** A page's folder is a one-level index shelved *mechanically* from a single facet (`domains[0]`, or an optional `shelf`), never by your own judgment. Each page's folder must match that facet; on divergence the frontmatter is authoritative and the file moves to match. A human moving a file by hand is a reclassification signal — sync the shelving facet to the new location, or ask if the move contradicts the classification. Never let a folder become a second source of truth.
