...
**Establish the two-branch model.** The project runs on `main` (released) + `develop` (integration) — see [git-conventions.md](../../../docs/wiki/git-conventions.md). After the initial commit exists on `main`, create `develop` if it's missing and make it the working base:
...
If a remote exists, push both: `git push -u origin main develop`. All subsequent work branches cut from `develop`.
...
- `docs/wiki/git-conventions.md` — the `main` + `develop` branch model, branch prefixes, commit format, and the LLM-owned merge/release flow (keep the shipped template content; only adjust if the project's flow genuinely differs).
...
**Stack:** <language>, <framework> | **Test:** `<test command>` | **Branches:** `main` (released) ← `develop` (integration) ← `feat/*`
...
| `/project:work`        | Top todo → branch off develop → red → green → refactor → commit → merge to develop (approved) |
| `/project:release`     | Merge develop → main, tag vX.Y.Z (approved)       |
...
6. **Agents own `docs/wiki/` and git state.** Humans browse and approve merges; agents write code, wiki, and run every git operation.
7. **Always branch before coding.** Cut from `develop`; never commit directly to `main` or `develop`.
8. **Conventional commits.** `feat:`, `fix:`, `chore:`, `docs:`, `test:`, `refactor:` (merge/release commits keep git's merge message).
...
14. **LLM-owned, human-approved merges.** The agent merges `feat/* → develop` (`--no-ff`) at the end of each cycle and `develop → main` via `/project:release` — both only after explicit human go-ahead. Never force-push `main`/`develop`.
```
...
Commit the scaffold **on `develop`** (the working base — `main` only moves by release), then push both long-lived branches:
