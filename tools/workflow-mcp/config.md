...
**What you set on a role is what it uses; a profile only matters where you did not set something.** A profile is a kind of role — `reasoning`, `balanced` or `fast`, roughly from most to least demanding — and each role's kind is the `profile:` line of its own file, `.agents/roles/<role>.md`, not `config.json`. The config holds the fallback: for each engine, which model and effort each kind gets. It exists so that a role which names no model of its own, or a fallback engine you never gave a row a model for, still gets a sensible one — and so that changing what `fast` means is one edit rather than one per role. **A row added in the editor is written with explicit values, so it never leans on a profile.** To move a role to another kind, edit its role file (and run `sync`, since the generated `AGENTS.md` lists it).
...
| `workerTimeoutSeconds` | whole number, 1–86400 | Seconds before a worker is stopped. **Only Antigravity workers use it today**; Claude and Codex workers ignore it. |
...
Prefixes rather than a list of names, so a newer model of a family fits the day it ships. They come from `modelPrefixes` (and `modelExcludes`) in `tools/workflow-mcp/engines/<engine>.mjs`; a tool that starts serving a family not covered — `agy models` lists Claude and GPT-OSS models beside its Gemini ones — gets it added there, on purpose. `inherit` and a blank are not models, so they fit every engine. A `model_override` on a dispatch is held to the same rule against the engine that dispatch resolves to: if the role's chain fell through to another engine, the override is refused with a message telling you to pass `cli_engine`, rather than launching that engine with a model it cannot run.

**The editor's model dropdowns are a convenience, not a whitelist of names — but they only ever offer the engine's own models.** Each one lists the models the engine's tool reports (see [Keeping model names up to date](#keeping-model-names-up-to-date)), then every id your config already uses for that engine, so an id you typed once can be picked everywhere; an id of another engine is never offered. The first entry is `default` — no pin, so the profile's default applies — and the last is **Other…**, which opens a box to type any id that engine runs. If the id you want is not listed, choose Other… and type it. Type another engine's id and the row turns red with why, and Save stays off. A wrong-engine model that is already in the file gets a red banner with **Clear it**, and the "Will run" line marks it ✖.
...
- **From your settings** — the value comes from this file: `--model` and `--effort` (per role and profile), the allowed commands (`workerCommands`), Antigravity's timeout (`workerTimeoutSeconds`), Codex's two token limits. Change these in the config.
