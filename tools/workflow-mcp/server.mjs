...
export function createServer(root, conductorEngine) {
  ...
  register('build_worker_prompt',
    ...
    + 'inspect_dispatch afterwards rather than judging the exit code and report by hand.',
    {
      ...
      lazy_skills: z.array(z.string()).optional().describe('Experimental. Skills from the ones this dispatch sends to name by path and description instead of inlining; the worker reads each from its own worktree when its description applies. Refused for a skill not sent, or one missing or different in the workspace. inspect_dispatch reports which were read where the engine has a transcript (agy).'),
