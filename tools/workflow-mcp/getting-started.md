...
1. **Plan (conditional).** If the todo is flagged `[complex]` or a batch of 2+ todos was proposed, `/project:work` dispatches the `planner` agent first; it's read-only and returns a stepwise plan in its report. `/project:work` saves it to `.handoff/<slug>-plan.md` (gitignored scratch) and passes that path as `instructions_file` — the MCP reads the file in the conductor's own checkout and inlines the text, so the developer receives the plan without the conductor having to re-type it through a tool call. A single simple todo skips planning.
...
3. **Run `/project:work`.** With `[complex]` set (or a 2+ batch), `/project:work` first dispatches the `planner`, which is read-only and returns a plan (following the `plan-writing` skill) in its report — goal, approach, ordered steps, risks, out-of-scope. `/project:work` sanity-checks it, then dispatches the `developer` with the plan, which drives the same Red → Green → refactor → wiki → commit flow as a simple feature, following the plan's step order.
4. **Where the plan lives.** In the planner's report — it writes no files itself. `/project:work` saves it to `.handoff/billing-invoices-plan.md`, gitignored scratch it clears when the cycle is done, and sends that path as `instructions_file`; the MCP reads it in the conductor's checkout and inlines the text, so the developer receives the plan and never a path (worktrees don't share scratch, so a path would be a file it could not open). The wiki holds the spec (what); the plan is how-to for one cycle. Because it isn't committed, a container recycle loses it — but so does it lose the rest of the uncommitted cycle, so `/project:work` simply restarts the still-open todo and re-dispatches the planner to regenerate the plan from the Behavior cases.
...
3. **The agent writes it.** `update-toolkit` produces `.agents/skills/database-migrations/SKILL.md` with frontmatter (precise `description` so future tasks auto-load it) and a procedural body — _how_ to do migrations in this project, not _what_ migrations are.
