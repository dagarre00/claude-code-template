...
2. **Pick the mailbox path.** `.handoff/<slug>-findings.md`. Gitignored scratch, same lifecycle as the plan file — it is a working surface for one review, not a record. `mkdir -p .handoff` if needed. Use exactly that name: the ignore glob matches only names ending in `-findings.md`, so a suffixed variant surfaces as untracked and fails the clean-tree gate at the end of the cycle.
...
9. **Clean up.** Delete `.handoff/<slug>-findings.md` and any plan scratch. Both are gitignored and nothing needs saving from them — the dispositions are in the commits and the work is in `todos.md`.
