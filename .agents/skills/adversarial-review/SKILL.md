...
description: How to run and answer an adversarial diff review in this project — dispatching the read-only adversary, the report format, the six-category sweep, severity vocabulary, and the triage protocol for each finding. Use when finishing a [complex] or batched cycle, or whenever a change needs a second set of eyes before commit. Trigger on "adversarial review", "second model", "review the diff", "findings", "triage findings", "red team the change", "before I call it done".
type: skill
---

# Adversarial Review — Dispatch, Mailbox, Triage

Runs over the commits just landed. A read-only `adversary` (fresh context, and by default a different model from the author's) reads them and returns numbered findings in its report; every one is triaged and the dispositions are recorded **in the commits that answer them**.

**Who triages.** The word "author" below means whoever is driving the cycle, and the split is fixed: the read-only **`triage`** role supplies a technical second opinion on each finding (is it real, does an invariant rule it out, what would the fix touch) — it needs neither the TDD procedure nor write access, so it is not the `developer`. The **`developer`** makes any approved fix, failing test first. **The dispatching conductor** — mid development cycle, or running an adversarial review on demand — owns every finding's severity and disposition, the `human-checkpoint` for `critical`/`major`, the todo lines, and the round-closing commit. A worker cannot hold the conversation with the human, so the checkpoint never belongs to one; and a severity is never lowered on the sole ground that a failure was not reproduced live.
...
1. **Fix the review scope.** The developer commits per case, so the scope is a commit range — typically `<sha-before-the-cases>...HEAD`, covering one case or a few closely-related ones. Not the whole branch, and not the whole cycle if the cycle was large. Nothing landed → skip and say so.

2. **Dispatch the `adversary`** with *only*:
   - **`diff_range` set to the range from step 1.** The MCP computes the diff and embeds it in the prompt as data; never just name the range in the instructions and expect the worker to resolve it. The adversary has no ranged `git diff` on its allowlist (`workerCommands` is a list of exact command lines, and a range carries a per-dispatch SHA that can never be a static literal), and its worktree holds the code *after* the change as ordinary files but no history. A reviewer left to fill that gap reads whole post-change files and infers what changed from commit subjects — measured, and it still returns valid findings, which is what makes this worth a rule rather than a note. Read the dispatch response before running the command: an empty or truncated range is reported there.
   - the entity slug(s) and the Behavior case IDs that range covers,
   - the test command from `docs/wiki/commands.md`.

   Findings come back **in the report**, not in a file. A read-only role runs in a read-only sandbox on the engines that can enforce one, so a scratch mailbox is a file it is not permitted to create: asked for one, the adversary reports "mailbox delivery blocked by the read-only filesystem" and the round produces nothing on disk. The report is the only channel that works on every engine, and it is the one the conductor reads anyway. Same shape as `plan-review`.
...
3. **Read the report.** If it lists no findings, it must still state what was checked — an unexplained pass is a failed review, so re-dispatch once with that instruction.
...
5a. **Check P0 saturation.** After staging the todo lines, count the open P0 items with the snippet in `docs/wiki/todos.md § P0 saturation threshold` — read that section, which also defines `P0_MAX` and says why the count covers all P0 items rather than only `[adversary]` ones.
...
6. **Re-dispatch only if something was fixed.** With findings filed rather than fixed, most rounds change no code and there is nothing to re-review — the review is done at step 6, and the queue owns the rest. If an approved fix did land, dispatch with `diff_range` over **the fix commits only** (`<sha-before-fixes>...HEAD`), never the original range — re-reading the whole thing is what manufactures fresh findings each round. It confirms the fix, contests rejections once, and stops.
...
8. **Clean up.** Delete any plan scratch left in `.handoff/`. The review itself leaves nothing behind — its findings were in a report and its dispositions are in the commits, and the work is in `todos.md`.

## Report format

What the adversary returns. It only has to survive long enough for you to triage from it, so keep it plain — the durable version of each finding is the one-line disposition you write into the commit.
...
- <pre-existing problems outside the diff — for the periodic whole-repo review, not this cycle>
```
...
| `nit`      | Style, naming, comment                                                  | **Not itemised** — tallied at the end of the report, never filed |
...
- **`nit`** — not written up at all. One tallied line at the end of the report (`Nits: 3 (naming ×2, stale comment ×1)`), and nothing filed.
...
- The conductor records the round in `log.md` as counts only (`Adversary: N findings — Fi filed, Fx fixed, R rejected`). The counts are an index; the dispositions themselves are in the commits, reachable with `git log --grep="adversary round"`. Do not write a separate review report; that is the periodic whole-repo review's artifact, not this one's.

## Swapping in an external reviewer

Cross-vendor independence is stronger than a second context on the same family. To use an external agent CLI (Codex or similar) as the adversary, keep the reporting contract identical and replace step 2 with a non-interactive invocation of that CLI in the repo root, briefed with the agent's own prompt plus: *"You are read-only: review and discuss only; do not edit files, commit, push, or reset the tree. Run `git rev-parse HEAD` and `git status` first and anchor every finding to that commit. Return findings as a numbered list in your final message."* Everything downstream — triage, re-review, stop condition — is unchanged. This project does not ship that dependency.
