...
- **Skill** = a procedure the agent runs, auto-loaded by task content. "Always do these steps when…". Lives in `.Codex/skills/<name>/SKILL.md`. This is almost always the right answer for new domain knowledge (progressive disclosure — domain knowledge belongs in skills the developer loads, not in new agents).
- **Command** = a human-invoked, named entry point that orchestrates work (branch, dispatch agents, touch the wiki). "The human types `/foo`". Lives in `.Codex/commands/project/<name>.md`. Commands stay thin — heavy lifting lives in agents and skills.
...
1. **Frontmatter.** `name` (kebab-case), `type: skill`, and a precise `description` — this is everything, because Codex uses it to auto-load. Bad: "skill for backend". Good: "Use when adding an HTTP endpoint. Trigger on 'add endpoint', 'new route', 'API handler'."
...
3. **Placement.** Every skill is a directory with a `SKILL.md` entrypoint: `.Codex/skills/<name>/SKILL.md`. A flat `.Codex/skills/<name>.md` or a grouping subfolder (`.Codex/skills/meta/<name>/SKILL.md`) is **silently ignored** — the skill never loads and nothing tells you why. Supporting files (templates, scripts) live next to `SKILL.md`.
...
- **Retire:** grep `.Codex/` and `docs/wiki/` for references, delete the `.Codex/skills/<name>/` directory, append to `docs/wiki/log.md`, commit `chore: retire <name> skill`.
...
4. **Placement.** `.Codex/commands/project/<name>.md`. The sub-folder is the namespace: `commands/project/work.md` → `/project:work`; a flat `commands/<name>.md` → `/<name>`. Keep new ones under `project:` unless you deliberately want un-namespaced.
5. **Update `AGENTS.md`** — add a row to the Slash commands table, including the argument the command accepts.
...
- **Modify:** re-read the file; if preconditions/output contract change, update `description` and the AGENTS.md row. If the argument's meaning changes, update `argument-hint`, the `**Argument:**` block, and the step that consumes it together — a hint that no longer matches the behaviour is worse than none. Commit `refactor: /<name> — <reason>`.
- **Retire:** grep `.Codex/` for references, delete the file, remove the AGENTS.md row, append to `docs/wiki/log.md`, commit `chore: retire /<name>`.
...
3. **Update `AGENTS.md`** — add a row to the Agent routing table.
...
- **Retire:** confirm no command references it (`grep -r "<agent-name>" .Codex/commands/`), delete the file, remove the AGENTS.md row, append to `docs/wiki/log.md`, commit `chore: retire <name> agent`.
