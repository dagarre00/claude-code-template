...
Copy this file to `.handoff/<slug>-handoff.md`, replace every `{{PLACEHOLDER}}`
...
It writes numbered findings to `.handoff/{{SLUG}}-findings.md`. If it comes back with nothing and does not say what it checked, re-dispatch it once with that instruction: an unexplained pass is a failed review.
...
Finally, delete the findings mailbox: `rm -f .handoff/{{SLUG}}-findings.md`. It is scratch; the dispositions live in the commits now.
...
3. Delete any other scratch you created under `.handoff/`.
...
Replace `<RANGE>`, `<CASE_IDS>`, `<ENTITY_PATH>`, `<TEST_CMD>`, `<MAILBOX_PATH>` (`.handoff/{{SLUG}}-findings.md`), and `<SLUG>` with the real values. Replace nothing else, and add nothing else.
...
- [ ] `{{HANDOFF_PATH}}` is deleted, along with any other scratch under `.handoff/`.
