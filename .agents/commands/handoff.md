...
1. **Pick the work.** Sync first: run the guarded sync block in `.agents/skills/feature-branching/sync-develop.md` (read it; its stop conditions apply). Then read `docs/wiki/todos.md` — the argument overrides the default. Skip `[wiki]` lines. Confirm against `origin/develop` that it has not already shipped.
...
4. **Build the brief** per the `llm-handoff` skill: copy `TEMPLATE.md` to `.handoff/<slug>-handoff.md`, fill every placeholder from the wiki verbatim, resolve inlined wikilinks, then run both verification greps — no surviving `{{...}}`, no vendor-specific vocabulary.
