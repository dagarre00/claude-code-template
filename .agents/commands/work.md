...
2. `build_worker_prompt` with the `role`, the `instructions`, the `workspace` from step 1, and — for a write role — `owned_paths` and a `commit_message`. Narrow `skills` to what that worker actually needs: this command declares eight, and sending all of them costs about 70KB against roughly 31KB for a realistic dispatch. If the task genuinely needs a file the worker cannot reach through its own worktree (a baseline outside it, a large fixture) — name it now and either inline its content in `instructions` or confirm the engine's read grant actually covers the path. Finding out from a denied-read in the report is a wasted round; a corrected resume once dropped 43KB → 27KB of prompt just from fixing this after the fact.
3. Run the returned `command` verbatim on a real POSIX shell (e.g. Git Bash on Windows). If your own shell tool cannot guarantee that — notably a Bash tool that might resolve to WSL — WSL cannot see a Windows drive path like `C:\...` the checkout actually lives at, so `cd` into the worktree fails before the worker ever runs. Use the structured `executable`/`args`/`cwd`/`stdin_file` fields instead and invoke the process directly; see the "workflow-mcp dispatch on Windows" gotcha.
4. Read the worker's report. **The worker delivers files and never runs git** — so for a write role, you stage its `owned_paths` in the worktree, commit with the message you passed, then integrate with a plain `git merge worker/<id>` (never `--ff-only` here). A worker's worktree is branched from whatever HEAD was current when you dispatched it, so if the integration branch has since moved — another case in the same batch, or a resumed session picking up a worktree left over from before — the merge is a real three-way merge, not a fast-forward, and that's expected: `--ff-only` is for syncing *your own* branch against an upstream you don't control (`feature-branching` skill), not for folding in a worker branch you dispatched yourself. Resolve any conflicts per `git-recovery` and keep the merge commit.
   **Removals are yours too, for the same reason.** A worker can create and edit files but cannot delete, move, or rename one — no `workerCommands` entry removes a file, and the write tools never unlink — so a task that moves a file splits in two: the worker writes the new path and reports the old one as superseded, and you `git rm` the old path when you stage. Do that in the same commit as the new file, and git records the change as a rename rather than a delete-plus-add, which is stronger evidence that the content survived the move than a worker-side `mv` would have been. A worker left to discover this rule ends its run on a denied action and costs you the whole dispatch.
