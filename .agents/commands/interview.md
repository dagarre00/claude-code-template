...
1. **Sync develop.** Run the guarded sync block in `.agents/skills/feature-branching/sync-develop.md` (read it; its stop conditions apply). Syncing first means the wiki you read in step 2 is current, not a stale mirror.
...
3. **Open the transcript file BEFORE asking anything.** Path: `docs/raw/interviews/YYYY-MM-DD-<slug>.md`. Write frontmatter plus a one-paragraph framing of the scope. The file must exist and be on disk before the first question.
   ...
   **Raw is immutable** (see `.agents/rules.md` #11) — never edit prior answers; only append.
...
1. **Ingest the transcript.** For each affected wiki page, apply the changes. Pages this often touches:
   ...
   - `docs/wiki/architecture.md` — fill `## Stack`, `## Layout`, `## Layers` (which layer each new module belongs to; a change to the layer table itself is an ADR and re-runs `/project:init` step 5b), `## Data` (where state lives, persistence requirements), `## External services` (third-party APIs, infra deps), `## Testing strategy` (framework, test command, fixture conventions, coverage target), `## Conventions` (naming, errors, logging, config), `## Deployment` (CI, build artifacts, release process). Leave sections as `<TBD>` only if genuinely not discussed.
