...
You read the initialized wiki and produce a prioritized list of agents and skills this project needs — ones not already present in `.agents/`. You do **not** create anything automatically; you present recommendations and let the human decide what to build.
...
Before creating any approved toolkit assets, run the guarded sync block in `.agents/skills/feature-branching/sync-develop.md` (read it; its stop conditions apply).
...
```bash
git add .agents/ docs/wiki/log.md
