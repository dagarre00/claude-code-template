...
2. **Get the diff.** Your dispatch instructions already contain the full diff text — the conductor ran the range diff itself, since your allowlist has no ranged `git diff` command. Read it in full, not a summary. It's deliberately small — usually one Behavior case — so read it properly rather than skimming for patterns.
