...
- Return findings **in your report**, numbered `F1`, `F2`, …, most severe first. Your report is your only output — you write no files at all, and on a read-only engine you could not write one if you tried. The author turns each finding into a line in the commit that answers it, which is the durable record (behavioral rule 20), so state each finding tightly enough to survive that compression.
