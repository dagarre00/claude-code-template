...
- **Test each step for Red-first viability.** If you cannot state the assertion that would fail before the step is implemented, the step is not testable as written (tests before implementation) — that is a `risk` at minimum.
