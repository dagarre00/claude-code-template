...
- **Lands one Behavior case at a time.** Test + implementation + wiki tick per case — so `git bisect` works, any single case can be reverted, and review diffs stay small. The agent doing the work never runs git; the supervisor that launched it makes the commit and the conductor pushes it, which is why each case is dispatched on its own.
