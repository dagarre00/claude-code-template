...
**`[infra]` extension:** a concept page backing an `[infra]` todo (`{{cmd:work}}` step 1) additionally carries a `## Behavior` section of verifiable operational assertions, with the same `[ ]`/`[~]`/`[x]` states as an entity page. That section is valid there — the lint pass must not flag it as off-template.
...
**Only for projects with a UI surface.** `{{cmd:init}}` creates this page when it detects one (web, mobile, desktop, TUI); a library, CLI, or service project never gets it. Do not create it speculatively — an empty design system on a backend project is the noise progressive disclosure exists to prevent.
...
A gap is a hole in the graph relative to this schema — computable by `{{cmd:wiki-lint}}` as a Bases/Dataview query — never "what feels missing". Don't fill gaps with invented prose: `status: stub` + `open_questions`, or ask the human.
...
through MCP after an explicit human maintenance request or `{{cmd:wiki-lint}}`.
