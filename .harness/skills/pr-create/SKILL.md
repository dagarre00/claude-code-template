---
name: pr-create
description: "How to draft and open a pull request for this project. Loads when a feature branch is finished — all Behavior cases [x] via {{cmd:work}}, or the human asks for a PR. Trigger on \"open PR\", \"create pull request\", \"PR template\", \"PR body\", \"draft PR\", \"feature complete\", \"all cases ticked\"."
---
...
Use this when wrapping up a `feat/*` or `fix/*` branch once the feature is complete. This skill is **automatically invoked by `{{cmd:work}}`** at the end of a cycle where all Behavior cases are `[x]`. Do not wait for an explicit PR request from the human.
