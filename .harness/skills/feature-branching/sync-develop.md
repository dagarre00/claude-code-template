...
The maintenance commands (`{{cmd:interview}}`, `{{cmd:review}}`, `{{cmd:wiki-lint}}`, `{{cmd:wiki-ingest}}`, `{{cmd:agent-scout}}`, `{{cmd:handoff}}`) run this before doing anything else. **Already on a `feat/*`/`fix/*` branch?** Stay there — living wiki edits ride the active branch (behavioral rule 19) — and skip this block.
