...
Not a stack skill. It says nothing about React, Tailwind, or SwiftUI — those belong in a `frontend-impl` skill that `{{cmd:agent-scout}}` recommends once the stack is real. This is the project-level procedure that sits above whatever the stack is.
