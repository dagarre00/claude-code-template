...
5. **Size check.** After appending, count the non-blank, non-header content lines in `gotchas.md`:
   ...
   ```
   - [ ] YYYY-MM-DD agent: gotchas.md has N field-lines — compact it during the next {{cmd:wiki-lint}}
   ```

   This keeps the file scannable before it degrades model attention. (`{{cmd:wiki-lint}}` compacts `gotchas.md`; there is no standalone prune command.)
