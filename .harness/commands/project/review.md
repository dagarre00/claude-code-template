---
name: review
description: "Thorough review of the codebase against the wiki. Runs the reviewer agent in a fresh session context with no developer baggage. Flags critical issues, warnings, drift, missing tests, security/perf concerns. Use periodically (~every 5 todos), never inside {{cmd:work}}."
...
Do **not** use `{{cmd:review}}` inside `{{cmd:work}}`. They're different phases.
...
4. **Process findings in the wiki.**
   ...
   - For each Missing ADR: queue the ADR for the next `{{cmd:work}}` cycle.
...
7. **Report to the human.** Highlight critical items only. Recommend whether the next step is `{{cmd:work}}` (fix critical), `{{cmd:interview}}` (spec gap), or `{{cmd:wiki-lint}}` (heavy drift).

## What you do NOT do

- **No code edits.** Findings only. The next `{{cmd:work}}` cycle fixes things.
- **No reviewer-in-`{{cmd:work}}`.** This is the cardinal violation — the `developer` cannot audit its own work.
