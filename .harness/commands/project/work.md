...
- An initialized wiki supplies a real todo, precise Behavior cases, and a runnable
  ...
  placeholder means stop and recommend `{{cmd:init}}` or `{{cmd:interview}}`.
...
1. **Select the work and contract.**
   ...
   - Skip `[wiki]` lines; they belong to `{{cmd:wiki-lint}}`.
...
12. **Report and surface maintenance cadence.** Summarize completed scope,
    ...
    Recommend—but never auto-run—periodic `{{cmd:review}}` after about five
    work cycles, or `{{cmd:wiki-lint}}` after five cycles, ten queued wiki items,
    or `FINDINGS_MAX` saturation. Count each cadence from its own last log entry.
    Recommend `{{cmd:agent-scout}}` when stack additions or repeated improvised
