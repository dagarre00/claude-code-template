...
- Last `{{cmd:wiki-lint}}` was > 5 work cycles ago.
- `{{cmd:review}}` flagged drift.
...
8. **Report to the human.** What was processed, what remains, gaps and contradictions detected — and the maintainer's **batched clarification questions in one lot** (contradictions, gaps needing knowledge outside `docs/raw/`, ambiguous merges). The human or `{{cmd:interview}}` resolves which version is correct; unresolved `contradicts` entries stay flagged until then.
...
- **Maintainer rewrites large sections of an entity page.** Preserve the result
  and ask before integration; content overhaul belongs to `{{cmd:interview}}`.
