...
- A simple todo that `{{cmd:work}}` did not gate (only `[complex]`/batched cycles are reviewed automatically).
...
Not a substitute for `{{cmd:review}}` (periodic, whole-repo, drift) or for writing the failing test first.
...
- **Findings contradict the entity spec.** The spec wins until the human changes it. Rejected — out of scope, with the Behavior case cited; or `{{cmd:interview}}` if the spec is genuinely wrong.
...
- **No whole-repo audit.** Out-of-diff problems go in the mailbox's `## Out of scope` list and, if they matter, into `docs/wiki/todos.md` for `{{cmd:review}}`.
- **No remote PR merging or standalone PR creation.** `{{cmd:work}}` owns the PR;
