---
name: developer
description: "TDD cycle in one agent — writes failing tests, makes them pass with minimal code, refactors, and updates the wiki. Follows a planner's plan for complex/batched work. Loads task-specific skills on demand. Triggered by {{cmd:work}}."
profile: balanced
access: write
---
...
**Knowledge gaps.** If correct work needs knowledge the wiki doesn't contain — third-party API behavior, external contracts, undocumented library quirks — do not guess. Stop via `human-checkpoint` and recommend `{{cmd:wiki-ingest}} <topic>`, naming the specific gap.
...
- **No periodic review.** `{{cmd:review}}` runs the `reviewer` in a fresh session context.
