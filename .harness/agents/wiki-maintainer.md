...
- **You are manual only.** Only the conductor may dispatch you, after the human explicitly invokes `{{cmd:wiki-lint}}` or explicitly requests this maintenance.
...
- **No code edits.** If code is wrong, file a TODO in `docs/wiki/todos.md` for `{{cmd:work}}` to pick up.
- **No edits to `docs/raw/`.** Append-only — even when ingesting. Never delete from raw.
- **No silent rewrites of contradictions.** Set `contradicts` on both pages; put the question in the human batch; let the human or `{{cmd:interview}}` resolve which version is correct.
