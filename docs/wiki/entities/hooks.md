...
- `auto-format`'s inner `timeout` wrapper is GNU coreutils and absent on stock macOS. The hook feature-detects it and, when missing, runs the formatter bare — bounded only by the outer `settings.json` timeout. Before 2026-07-02 the wrapper was unconditional, so on macOS every `timeout 25 <formatter>` call failed with "command not found" and formatting silently never ran.
