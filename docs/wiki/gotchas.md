...
**Fix:** Every agy worker now launches as a custom agent with `excludeDefaultComponents: true` (`tools/workflow-mcp/engines/antigravity.mjs`), which removes the artifacts section that invites the argument. `antigravity` is still off the `developer` chain in `.agents/config.json` — putting it back is a project decision, now supported by 8 of 8 clean developer dispatches in the 2026-09-13 e2e rather than blocked by an unexplained failure. If a write role ever hits this again, the wrapped command exits non-zero on the unfinished run; read the report before re-dispatching.
**Related:** [[decisions/2026-09-08-drop-commands-from-mcp-surface]]
