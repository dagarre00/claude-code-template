...
- **One summary per raw file** in `docs/wiki/summaries/`, written by `/project:wiki`. The `wiki-maintainer` catches stragglers during `/project:wiki` — raw files that never got a summary.

See `.agents/commands/wiki.md` for the ingest procedure, and `.agents/roles/wiki-maintainer.md` for the straggler pass.
